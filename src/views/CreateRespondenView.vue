...
const rules   = computed(()=>{
  return {
    ...
    kelompok : { required : helpers.withMessage('Kelompok harus dipilih salah satu',required) },
    password_responden : { required : helpers.withMessage('Password Responden harus diisi',required) },
    password_confirm : {
    	required : helpers.withMessage('Password Confirm harus diisi',required),
    	sameAsPassword : helpers.withMessage('Password Confirm tidak sama dengan Password',sameAs(form.password_responden))
    }
  }
})
...
async function onSubmit(){
    ...
    if (!v$.value.$error) {
        Swal.showLoading()
        try{
            var response = await axios.post(CONSTANT.serverURL+'create-responden',form,{
                headers : {
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="input-group">
	                                        ...
	                                        <div class="invalid-feedback" style="display:block;" v-if="v$.password_responden.$error">{{v$.password_responden.$errors[0].$message}}</div>
                                        </div>
                                    </div>
                                </div>
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="input-group">
                                            ...
                                            <div class="invalid-feedback" style="display:block;" v-if="v$.password_confirm.$error">{{v$.password_confirm.$errors[0].$message}}</div>
                                        </div>
                                    </div>
                                </div>
                                <div class="row mb-3">
